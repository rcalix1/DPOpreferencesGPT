...
class inferenceGPT:
    ...
    def convert_preferences_to_tensor_batches(self, preference_data):
        inputs, preferreds, rejecteds = [], [], []

        for item in preference_data:
            try:
                inputs.append(torch.tensor(item["input"], dtype=torch.float32))
                preferreds.append(torch.tensor(item["preferred"], dtype=torch.float32))
                rejecteds.append(torch.tensor(item["rejected"], dtype=torch.float32))
            except Exception as e:
                print(f"⚠️ Skipping bad sample: {e}")

        print(f"✅ Loaded {len(inputs)} valid preference samples into tensors.")
        return (
            torch.stack(inputs), 
            torch.stack(preferreds), 
            torch.stack(rejecteds)
        )
    ...
    def dpo_finetune_with_kl(self, model, base_model, preference_data, optimizer=None, device="cuda", epochs=3, beta=0.05):
        ...
        for epoch in range(epochs):
            ...
            for item in preference_data:
                ...
                pred_new = pred_new_full[:, -9:, :]  # [1, 9, 35]

                # Compute MSE scores — smaller = better
                pref_score = F.mse_loss(pred_new, preferred_tensor[:, -9:, :], reduction='mean')
                rej_score  = F.mse_loss(pred_new, rejected_tensor[:, -9:, :], reduction='mean')

                # Preference loss: prefer lower MSE (closer to preferred)
                preference_loss = F.softplus(rej_score - pref_score)

                # Generate old prediction from base model
                with torch.no_grad():
                    pred_old_full, _ = base_model.generate(input_tensor, max_new_tokens=9, reasoning_steps=10)
                    pred_old = pred_old_full[:, -9:, :]

                # KL proxy: L2 distance between predictions
                kl_term = F.mse_loss(pred_new.detach(), pred_old.detach(), reduction='mean')

                # Final combined loss
                beta = 1.0
                total_dpo_loss = preference_loss + beta * kl_term
    ...
    def dpo_finetune_with_kl_last(self, model, base_model, preference_data, optimizer=None, device="cuda", epochs=3, beta=0.05):
        ...
        for epoch in range(epochs):
            ...
            for item in preference_data:
                ...
                logp_pref_new = -F.mse_loss(pred_new, preferred_tensor[:, -9:, :], reduction='mean')   ## mean or sum
                ...
                with torch.no_grad():
                    ...
                    logp_pref_old = -F.mse_loss(pred_old , preferred_tensor[:, -9:, :],  reduction='mean')
                ...
                # Final combined loss
                beta = 1.0     ## 0.05    ## 0.05    ## 0.01 or 0.001
    ...
    ## batched version - for spped up
    def dpo_finetune_with_kl_no(
        self, 
        model, 
        base_model, 
        inputs, 
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, N, batch_size):
                ...
                input_batch     = inputs[idx].to(device)[:, :10, :]       # [B, 10, F]
                preferred_batch = preferreds[idx].to(device)[:, -9:, :]   # [B, 9, F]
                rejected_batch  = rejecteds[idx].to(device)[:, -9:, :]    # [B, 9, F]

                # === Generate predictions ===
                pred_new, _ = model.generate(input_batch, max_new_tokens=9, reasoning_steps=10)
                pred_new = pred_new[:, -9:, :]  # [B, 9, F]

                with torch.no_grad():
                    pred_old, _ = base_model.generate(input_batch, max_new_tokens=9, reasoning_steps=10)
                    pred_old = pred_old[:, -9:, :]  # [B, 9, F]

                # === Reward scores (neg MSE as proxy for log-prob) ===
                logp_pref_new = -F.mse_loss(pred_new, preferred_batch, reduction='none').mean(dim=[1, 2])  # [B]
                logp_rej_new  = -F.mse_loss(pred_new, rejected_batch,  reduction='none').mean(dim=[1, 2])  # [B]

                logp_pref_old = -F.mse_loss(pred_old, preferred_batch, reduction='none').mean(dim=[1, 2])  # [B]
                logp_rej_old  = -F.mse_loss(pred_old, rejected_batch,  reduction='none').mean(dim=[1, 2])  # [B]

                # === DPO Loss ===
                dpo_loss = F.softplus(logp_rej_new - logp_pref_new).mean()

                # === KL Term ===
                kl_pref = logp_pref_new - logp_pref_old  # [B]
                kl_rej  = logp_rej_new  - logp_rej_old   # [B]
                kl_term = (kl_pref + kl_rej) / 2         # [B]
                kl_loss = kl_term.mean()

                # === Final Loss ===
                beta = 0
                total_dpo_loss = dpo_loss + beta * kl_loss
            ...
            print(f"[Epoch {epoch+1}] Batched DPO Loss (with KL, β={beta}) = {total_loss:.4f}")
    ...
    def DPO_RLHF(self, si_GPT):
        ...
        ## inputs, preferreds, rejecteds = self.convert_preferences_to_tensor_batches(prefs)  ## speed up REMOVE
        ...
        '''
        model_DPO = self.dpo_finetune_with_kl(
            model=si_GPT,
            base_model=copy.deepcopy(si_GPT).eval(),
            inputs=inputs,
